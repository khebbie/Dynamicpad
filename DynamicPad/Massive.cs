    ...
    public static class ObjectExtensions
    {
        ...
        public static dynamic ToExpando(this object o)
        {
            ...
            if (o.GetType() == typeof (NameValueCollection) || o.GetType().IsSubclassOf(typeof (NameValueCollection)))
            {
                ...
                nv.Cast<string>().Select(key => new KeyValuePair<string, object>(key, nv[key])).ToList().ForEach(
                    i => d.Add(i));
            }
            else
            {
    ...
    public class DynamicModel : DynamicObject
    {
        ...
        public dynamic Prototype
        {
            get
            {
                ...
                IEnumerable<dynamic> schema = Schema;
        ...
        public dynamic CreateFrom(NameValueCollection coll)
        {
            ...
            {
                bool exists = schema.Any(x => x.COLUMN_NAME.ToLower() == item.ToString().ToLower());
                if (exists)
                {
        ...
        public virtual IEnumerable<dynamic> Query(string sql, params object[] args)
        {
            ...
            {
                DbDataReader rdr = CreateCommand(sql, conn, args).ExecuteReader();
                while (rdr.Read())
                {
        ...
        {
            using (DbDataReader rdr = CreateCommand(sql, connection, args).ExecuteReader())
            {
                while (rdr.Read())
                {
                    yield return rdr.RecordToExpando();
        ...
        /// </summary>
        public virtual IEnumerable<dynamic> All(string where = "", string orderBy = "", int limit = 0,
        ...
        private static string BuildSelect(string where, string orderBy, int limit)
        {
            ...
            if (!String.IsNullOrEmpty(orderBy))
                sql += orderBy.Trim().StartsWith("order by", StringComparison.OrdinalIgnoreCase)
                           ? orderBy
                           : " ORDER BY " + orderBy;
            return sql;
        }
        ...
        {
            dynamic result = new ExpandoObject();
            string countSQL = string.Format("SELECT COUNT({0}) FROM {1} ", PrimaryKeyField, TableName);
            if (String.IsNullOrEmpty(orderBy))
            ...
            string sql =
                string.Format(
                    "SELECT {0} FROM (SELECT ROW_NUMBER() OVER (ORDER BY {2}) AS Row, {0} FROM {3} {4}) AS Paged ",
        ...
        {
            string sql = string.Format("SELECT {0} FROM {1} WHERE {2} = @0", columns, TableName, PrimaryKeyField);
        ...
        public virtual IDictionary<string, object> KeyValues(string orderBy = "")
        {
            ...
            string sql = string.Format("SELECT {0},{1} FROM {2} ", PrimaryKeyField, DescriptorField, TableName);
            if (!String.IsNullOrEmpty(orderBy))
        ...
        {
            IDictionary<string, object> dc = ItemAsDictionary(item);
        ...
        public virtual int Save(params object[] things)
        {
            ...
            List<DbCommand> commands = BuildCommands(things);
        ...
        public virtual DbCommand CreateInsertCommand(dynamic expando)
        {
            ...
            if (counter > 0)
            {
                ...
                string sql = string.Format(stub, TableName, keys, vals);
                result.CommandText = sql;
            }
            else
                throw new InvalidOperationException(
        ...
        public virtual DbCommand CreateUpdateCommand(dynamic expando, object key)
        {
            var settings = (IDictionary<string, object>) expando;
            var sbKeys = new StringBuilder();
            string stub = "UPDATE {0} SET {1} WHERE {2} = @{3}";
            ...
            if (counter > 0)
            {
                ...
                string keys = sbKeys.ToString().Substring(0, sbKeys.Length - 4);
        ...
        public virtual dynamic Insert(object o)
        {
            ...
            if (BeforeSave(ex))
            {
                ...
                {
                    dynamic cmd = CreateInsertCommand(ex);
                    cmd.Connection = conn;
                    cmd.ExecuteNonQuery();
        ...
        public override bool TryInvokeMember(InvokeMemberBinder binder, object[] args, out object result)
        {
            ...
            if (info.ArgumentNames.Count > 0)
            {
                ...
                {
                    string name = info.ArgumentNames[i].ToLower();
                    switch (name)
                    {
                        case "orderby":
            ...
            else
            {
                ...
                bool justOne = op.StartsWith("First") || op.StartsWith("Last") || op.StartsWith("Get") ||
                               op.StartsWith("Single");
